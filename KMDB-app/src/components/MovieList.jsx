  ...
  return (
    <Box sx={{ overflowX: "auto", display: "flex", gap: 2, pt: 2 }}>
      {items.map((item) => {
        ...
        return <MovieCard id={id} title={title} date={date} score={score} posterUrl={posterUrl} mediaType={mediaType} fullPosterUrl={item.poster_path ? `https://image.tmdb.org/t/p/w780${item.poster_path}` : null} />;
      })}
    </Box>
  );
};
